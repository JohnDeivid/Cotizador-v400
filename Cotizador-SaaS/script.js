    ...
    // =========================================================================
    // Las credenciales ahora están securizadas en el backend a través de Netlify Functions.
    ...
    function abrirModal(categoria) {
      ...
      document.getElementById('modal-search').value = ''; // Limpiar buscador al abrir
    ...
    // Nueva función: Filtro rápido en el modal
    ...
    // =========================================================================
    // 6. EXPORTACIONES (PDF, Excel, WhatsApp)
    // =========================================================================
    ...
    async function aprobarCotizacion() {
      ...
      // 3. Preparar e invocará API de Airtable
      ...
      try {
        ...
        if (!resp.ok) {
          ...
          alert("Error al guardar en Airtable. Ojo a este detalle: \n\n" + errText);
        } else {
          alert("¡Gracias! Tu cotización ha sido guardada exitosamente y pronto te contactaremos.");
          cambiarPaso(1); // Volver al inicio para que todo regrese a la normalidad

          // Opcional: podrías resetear el carrito aquí si lo deseas
          // carrito = [];
          // renderCarrito();
        }
      } catch (err) {
        alert("Error de red: ¿Acabas de probar dándole doble clic al index.html en tu PC? Recuerda que esto SOLO funciona si lo subes a Netlify y entras por el link.");
